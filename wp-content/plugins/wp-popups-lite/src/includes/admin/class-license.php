...
class WPPopups_License {
	...
	public function verify_key( $key = '', $option_name = 'wppopups_license', $item_id = 0, $ajax = false, $forced = false ) {
		...
		if ( is_array( $verify ) && isset( $verify['error'] ) ) {
			$msg = esc_html__( 'There was an error connecting to the remote key API. Please try again later.', 'wppopups-pro' ) . esc_html( $verify['error'] );
			if ( $ajax ) {
	...
	public function deactivate_key( $ajax = false, $option_name = 'wppopups_license' ) {
		...
		if ( is_array( $deactivate ) && isset( $deactivate['error'] ) ) {
			$msg = esc_html__( 'There was an error connecting to the remote key API. Please try again later.', 'wppopups-pro' ) . esc_html( $deactivate['error'] );
			if ( $ajax ) {
				wp_send_json_error( $msg );
			} else {
				$this->errors[] = $msg;
