 ...
 */
class Music_Zone_Dropdown_Taxonomies_Control extends WP_Customize_Control {
 ...
 * @see WP_Customize_Control
 */
class Music_Zone_Dropdown_Category_Control extends WP_Customize_Control {
...
class Music_Zone_Note_Control extends WP_Customize_Control {
...
class Music_Zone_Switch_Control extends WP_Customize_Control{
	public $type = 'switch';
...
class Music_Zone_Dropdown_Chooser extends WP_Customize_Control{
	...
	public function render_content(){
		...
		?>
            <label>
                ...
                <select class="music-zone-chosen-select" <?php $this->link(); ?>>
                    <?php
...
class Music_Zone_Dropdown_Multiple_Chooser extends WP_Customize_Control{
	...
	public function render_content(){
		...
		?>
            <label>
                ...
                <select class="music-zone-chosen-select" multiple <?php $this->link(); ?>>
                    <?php
                    foreach ( $this->choices as $value => $label )
                        echo '<option value="' . esc_attr( $value ) . '"' . selected( $this->value(), $value, false ) . '>' . esc_html( $label ) . '</option>';
                    ?>
 ...
 */
class Music_Zone_Custom_Radio_Image_Control extends WP_Customize_Control {
...
class Music_Zone_Customize_Sortable_Control extends WP_Customize_Control {
  ...
  public function render_content() {
    ...
    <ul class="music-zone-sortable-list">
      ...
      <?php
       foreach ( $values_choices as $value ){
        ...
        $trimmed_value = preg_replace('/\s+/', '', $strtolower);
       	?>
	        <li>
	            <input class="music-zone-sortable-input sortable-hideme" name="<?php echo esc_attr( $trimmed_value ); ?> " type="hidden"  value="<?php echo esc_attr( $trimmed_value ); ?>" />
	            <span class ="menu-item-handle sortable-span"> <?php echo esc_attr( $choices[$value] ); ?> </span>
	          <i class="dashicons dashicons-menu music-zone-drag-handle"></i>
	        </li>
        ...
        <li class="sortable-hideme">
          <input class="music-zone-sortable-value" <?php $this->link(); ?> value="<?php echo esc_attr( $this->value() ); ?>" />
        </li>
...
class Music_Zone_Customize_Horizontal_Line extends WP_Customize_Control {
 ...
 * @since  1.0.0
 */
class Music_Zone_Multi_Input_Custom_Control extends WP_Customize_Control {
	/**
	 * Control type
	 *
	 * @var string
...
class Music_Zone_Icon_Picker extends WP_Customize_Control{
	...
	public function render_content(){
		...
		?>
            <label>
                ...
                <input id="music-zone-<?php echo esc_attr( $id ); ?>" placeholder="<?php esc_attr_e( 'Click here to select icon', 'music-zone' ); ?>" class="music-zone-icon-picker input" <?php $this->link(); ?> value="<?php echo esc_attr( $this->value() ); ?>" />
            </label>
		<?php
	}
}

class Music_Zone_Multiple_Dropdown_Chooser extends WP_Customize_Control{
	public $type = 'dropdown_chooser';
	...
	public function render_content(){
		...
		?>
            <label>
                ...
                <select class="music-zone-chosen-select" <?php $this->link(); ?> multiple>
                    <?php
                    foreach ( $this->choices as $value => $label )
                        echo '<option value="' . esc_attr( $value ) . '"' . selected( $this->value(), $value, false ) . '>' . esc_html( $label ) . '</option>';
                    ?>
...
class Music_Zone_Customize_Column extends WP_Customize_Control {
	public $type = 'hr';
